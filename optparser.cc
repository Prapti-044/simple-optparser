...
bool generateDOT(ostream &out) {
  ...
  for (CodeObject::funclist::iterator fit = funcs.begin(); fit != funcs.end();
       fit++) {
    ParseAPI::Function *f = *fit;
    if (f->blocks().empty()) continue;

    for (ParseAPI::Function::blocklist::iterator b = f->blocks().begin();
         b != f->blocks().end(); b++) {
      Block *block = *b;
  ...
  for (CodeObject::funclist::iterator fit = funcs.begin(); fit != funcs.end();
    ...
    for (ParseAPI::Function::blocklist::iterator b = f->blocks().begin();
         b != f->blocks().end(); b++) {
      Block *block = *b;
      for (ParseAPI::Block::edgelist::const_iterator e =
               block->sources().begin();
...
bool generateLineInfo(ostream &out, bool sourcelines_only = false) {
  ...
  for (set<Address>::iterator addri = addresses.begin();
       addri != addresses.end(); addri++) {
    cur_lines.clear();
    ...
    for (vector<Statement::Ptr>::iterator linej = cur_lines.begin();
         linej != cur_lines.end(); linej++) {
      Statement::Ptr fl = *linej;
      unique_sourcefiles.insert(fl->getFile());
    }
  }

  if (sourcelines_only) return true;

  if (all_lines.empty()) return true;
...
void printFnVars(ostream &out, FunctionBase *f, bool isInlineFn, int tabWidth) {
  ...
  for (set<localVar *>::iterator it = allVars.begin(); it != allVars.end();
       ++it) {
    for (int t = 0; t < tabWidth + 1; t++) {
      out << "\t";
    }
...
void printInlineEntries(ostream &out, set<InlinedFunction *> &ifuncs,
                        int tabwidth) {
  ...
  for (set<InlinedFunction *>::iterator i = ifuncs.begin();;) {
    ...
    for (int t = 0; t < tabwidth; t++) out << "\t";
    out << "  \"callsite_file\": \"" << ifunc->getCallsite().first << "\","
        << endl;
    for (int t = 0; t < tabwidth; t++) out << "\t";
    ...
    for (SymtabAPI::InlineCollection::iterator j = ic.begin(); j != ic.end();
         j++)
...
void printInlines(ostream &out, ParseAPI::Function *f) {
  ...
  //  in the binary forms a functional-style unit).  Optimizations like
  //  outlining or multi-entry functions can cause this.
  // For each basic block in the ParseAPI function, get the containing SymtabAPI
  //  function and use the set of SymtabAPI functions to get inlining for this
  //  parse function.
  set<FunctionBase *> top_level_functions;
  for (ParseAPI::Function::blocklist::iterator i = f->blocks().begin();
       i != f->blocks().end(); i++) {
  ...
  for (set<FunctionBase *>::iterator i = top_level_functions.begin();
       i != top_level_functions.end(); i++) {
...
static void printSourceFiles(ostream &out) {
  ...
  for (set<string>::iterator i = unique_sourcefiles.begin();
       i != unique_sourcefiles.end();) {
    string file = *i;
    out << "\t{ \"file\": \"" << file << "\" }";
    i++;
...
void setBlockIds() {
  ...
  for (CodeObject::funclist::iterator f = funcs.begin(); f != funcs.end();
       f++) {
    ParseAPI::Function::blocklist blocks = (*f)->blocks();
    for (ParseAPI::Function::blocklist::iterator i = blocks.begin();
         i != blocks.end(); i++) {
...
static void restartTimeout() { alarm(INTERACTIVE_TIMEOUT_S); }

static void startInteractiveSession(string &cmd, ifstream &in, ofstream &out,
                                    ofstream &ctrl) {
  for (;;) {
    bool had_error = true;
    restartTimeout();
